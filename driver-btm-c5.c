...
{
	int i, j;
	int default_freq = 600;
	int default_freq_index = get_pll_index(default_freq);
	int max_freq_index = -1;

	/* iterate over all chains and try to load defaults from flash */
	...
	for (i = 0, chain_id = 0; i < BITMAIN_MAX_CHAIN_NUM; i++) {
		if (dev->chain_exist[i] == 1 && dev->chain_asic_num[i] > 0) {
			...
			applog(LOG_NOTICE, "chain %d (id=%d): last_freq flash %svalid, badcore flash %svalid",
				i, chain_id, lastfreq_flash_valid ? "" : "not ", badcore_flash_valid ? "" : "not ");

			/* by default there are no bad cores */
			for (j = 0; j < CHAIN_ASIC_NUM; j++)
				chain_badcore_num[i][j] = 0;

			/* load data from flash? */
			if (lastfreq_flash_valid) {
				applog(LOG_NOTICE, "chain %d: reading flash", i);
				/* yes, load everything */
				pic_temp_offset[i] = ((last_freq[i][2]&0x0f)<<4) + (last_freq[i][4]&0x0f);
				base_freq_index[i] = ((last_freq[i][6]&0x0f)<<4) + (last_freq[i][8]&0x0f);

				applog(LOG_NOTICE, "chain %d: base_freq=%d temp_offset=%d",
					i, get_freqvalue_by_index(base_freq_index[i]), pic_temp_offset[i]);
				for (j = 0; j < dev->chain_asic_num[i]; j++) {
					int step_down = last_freq[i][0] & 0x3f;
					if (step_down == 0x3f)
						step_down = 0;
					if (step_down > 0)
						applog(LOG_NOTICE, "chain %d chip %d has step_down %d", i, j, step_down);
					last_freq[i][j*2+3] -= step_down;
				}

				if (badcore_flash_valid) {
					for (j = 0; j < CHAIN_ASIC_NUM; j++) {
						if (j%2)
							chain_badcore_num[i][j] = (badcore_num_buf[i][(j/2)*2+1]&0x0f);
						else
							chain_badcore_num[i][j] = (badcore_num_buf[i][(j/2)*2+1]>>4)&0x0f;
						if (chain_badcore_num[i][j] > 0)
							applog(LOG_NOTICE, "chain %d chip %d has badcore %d",
			...
			for (j = 0; j < CHAIN_ASIC_NUM; j++) {
				if (last_freq[i][j*2+3] < MIN_FREQ)
					last_freq[i][j*2+3] = MIN_FREQ;
				if (last_freq[i][j*2+3] > MAX_FREQ)
					last_freq[i][j*2+3] = MAX_FREQ;
	...
	for (i = 0; i < BITMAIN_MAX_CHAIN_NUM; i++) {
		if (dev->chain_exist[i] == 1 && dev->chain_asic_num[i] > 0) {
			...
			int first_chip_freq_index = last_freq[i][0*2 + 3];
			int different_freqs = 0;
			for (j = 0; j < dev->chain_asic_num[i]; j++) {
				if (last_freq[i][j*2 + 3] != first_chip_freq_index)
					different_freqs = 1;
			...
			if (!different_freqs && first_chip_freq_index != base_freq_index[i])
				applog(LOG_NOTICE, "chain %d has chips of frequency %d but base_freq is %d",
					i, get_freqvalue_by_index(first_chip_freq_index),
					get_freqvalue_by_index(base_freq_index[i]));
			for (j = 0; j < dev->chain_asic_num[i]; j++)
				applog(LOG_DEBUG, "chain %d chip %d: frequency %d",
					i, j, get_freqvalue_by_index(last_freq[i][j*2 + 3]));
		}
	}

	/* remember max frequency for voltage setting */
	dev->frequency = get_freqvalue_by_index(max_freq_index);
	applog(LOG_NOTICE, "max frequency = %d", dev->frequency);
}


    void set_frequency0(unsigned short int frequency)
    {
        unsigned char buf[9] = {0,0,0,0,0,0,0,0,0};
        unsigned int cmd_buf[3] = {0,0,0};
...
void bitmain_reconfigure_fans(void)
{
	...
	if (opt_fan_ctrl == FAN_MODE_TEMP) {
		...
		applog(LOG_NOTICE, "EMERGENCY fan control, fans to full");
		...
		if (dev->chain_exist[i] == 1) {
			...
			if (set_vol != 0) {
				applog(LOG_NOTICE,"chain %d: overriding global voltage from default %.2lfV to %.2lfV",
						i, chain_voltage_value[i] / 100.0, set_vol/ 100.0);
				chain_voltage_value[i] = set_vol;
				chain_voltage_pic[i] = getPICvoltageFromValue(chain_voltage_value[i]);
			}
			chain_id++;
		}
